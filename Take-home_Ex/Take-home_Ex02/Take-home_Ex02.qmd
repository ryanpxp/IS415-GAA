...
```{r}
overall_drugs <- left_join(thai, drugs, by = c("ADM1_EN" = "province_en")) %>%
  dplyr::select(1:3, 17:19,21) %>%
  filter(types_of_drug_offenses %in% confirmed_drug_offenses)
```
...
for (year in 2017:2022) {
  ...
  # Summarize the data for each year by province (one row per province)
  drugs_summarized <- drugs_by_year[[as.character(year)]] %>%
    group_by(ADM1_EN) %>%
    summarise(no_cases = sum(no_cases, na.rm = TRUE)) %>%
    ungroup()
  
  # Perform the left join with the summarized data
  map_data <- left_join(thai, drugs_summarized, by = c("ADM1_EN"))
...
![Close-up of neighbour plot](img\Neighbour_phuket_plot.png){width="100%" style="object-fit: contain;"}
