...
Since we will only focus on drug abuse, we would be only extracting the "drug_use_cases" from the types of drug offenses.We will also break up the data into years for visualisation.
...
![Phuket Region](data\raw\Phuket_region.png){width="100%" style="object-fit: contain;"}
...
```
From the code above, we now know the number for the ***Phangnga** region and we will manually add **Phuket** to **Phangnga** and **Phangnga** to **Phuket**.
